-- INFO: just a copy --> do not look at this
      ...
      LOOP
         ...
         if minor_queue_id = 1 then -- non-horizontal  DS, AC
            non_horizontal : FOR I in 4..5 LOOP
               ...
               -- or next task is AC
               if (period_cnt > estimate_time)
                 or (Self.major_queue(I).execute( Self.major_queue(I).id) = AC_id_que_2) then
                  Answer := self.major_queue(I).execute( self.major_queue(I).deadline );
               else -- start queue noraml behavior again, i.e. run AC
                  minor_queue_id := 0; --
                  exit non_horizontal;
               end if;
               ...
               if (Answer = 1) then
                  minor_queue_id := 0; -- good value --> start normal behavior
                  exit non_horizontal when Answer = 1; -- exit when good value
               end if;
         ...
         if minor_queue_id = 2 then -- Sensor Check  DS, SS
            sensor_check : FOR I in 6..7 LOOP
               ...
               -- or next task is AC
               if (period_cnt > estimate_time)
                 or (Self.major_queue(I).execute( Self.major_queue(I).id) = 1) then
                  Answer := self.major_queue(I).execute( self.major_queue(I).deadline );
               else -- start queue normal behavior again, i.e. run AC
                  minor_queue_id := 0; --
                  exit sensor_check;
               end if;
               ...
               if (Answer = 1) then
                  minor_queue_id := 0; -- good value --> start normal behavior
                  exit sensor_check when Answer = 1; -- exit when good value
               end if;
